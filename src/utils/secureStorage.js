...
export async function saveSecureSettings(settings) {
  try {
    ...
    localStorage.setItem('betternotepad-ai-settings', JSON.stringify(secureSettings));

    // Mark as encrypted for future detection
    localStorage.setItem('betternotepad-ai-settings-encrypted', 'true');
  } catch (error) {
...
export async function loadSecureSettings() {
  try {
    const saved = localStorage.getItem('betternotepad-ai-settings');
    if (!saved) return null;

    const settings = JSON.parse(saved);
    const isEncrypted = localStorage.getItem('betternotepad-ai-settings-encrypted') === 'true';
...
export function clearSecureSettings() {
  localStorage.removeItem('betternotepad-ai-settings');
  localStorage.removeItem('betternotepad-ai-settings-encrypted');
}
